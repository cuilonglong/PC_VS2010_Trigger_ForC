...
注：目前仅支持8bit加解密
*/
int DES::DataAlgorithm(char mode,unsigned char *key,int keylen,unsigned char *data,int datalen) 
{
	int output[72]={0};
	char data1[9],key1[9];
	char out[9]={0};

	if((datalen != 8)||(keylen != 8))
	{
		return -1;
	}
	...
	if(mode == 0)//加密
	{
		DES_Efun(data1,key1,output);  //加密
		BitToChar(output,out,8);
	}
	else if(mode == 1)//解密
	{
		CharToBit(data1,output,8);
		DES_Dfun(output,key1,out);//解密
	}
	memcpy(data,out,8);
	return 0;
}
